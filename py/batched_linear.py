...
def _extract_from_and_to(clone_mask, clone_indices, batch_size):
    """
    Extract indices for copy/blend operations.

    Semantics:
    - `mask[i] == True` means slot `i` will be overwritten (destination).
    - `indices[i]` gives the source slot to read from for that destination.
    """
    ...
    dest = torch.arange(batch_size, device=clone_indices.device)[clone_mask]
    src = clone_indices[clone_mask]

    # Sanity check: all dest must be in valid range
    if not torch.all((0 <= dest) & (dest < clone_mask.shape[0])):
...
class BatchedLinear(BatchedNNModule):
    ...
    def clone(self, clone_mask: torch.Tensor, clone_indices: torch.Tensor):
        ...
        dest, src = _extract_from_and_to(clone_mask, clone_indices, self.batch_size)
        self.weight[dest] = self.weight[src]
        if self.bias is not None:
            self.bias[dest] = self.bias[src]
    ...
    def blend(self, mask: torch.Tensor, indices: torch.Tensor):
        ...
        dest, src = _extract_from_and_to(mask, indices, self.batch_size)
        ...
        self.weight[dest] = (self.weight[dest] + self.weight[src]) / 2
        if self.bias is not None:
            self.bias[dest] = (self.bias[dest] + self.bias[src]) / 2
